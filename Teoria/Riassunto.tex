...
Rappresenta:
\begin{itemize}
    \item tipi di oggetti (classi) del sistema, corrispondono alle entità esistenti nel sistema.
    \item relazioni statiche tra classi ed i vincoli che si applicano a tali relazioni.
    \item caratteristiche delle classi: proprietà (attributi e responsabilità dei metodi) e responsabilità.
\end{itemize}

\subsubsection{Utilizzo del diagramma}
\begin{itemize}
    \item Fase di definizione dei requisiti: vede le \textbf{entità} del dominio e cattura i suoi concetti principali, fissando anche i confini del sistema. Modello architetturale rivolto alla comprensione di cosa fa il sistema, non di come lo fa.
    \item Fase di progettazione: identifica le \textbf{identità} del sistema e le relazioni tra esse, prescinde dall'implementazione.
    \item Fase di implementazione: identifica \textbf{classi} del software e le relazioni tra esse, corrisponde alla struttura reale del software poiché ne modella l'implementazione OO.
\end{itemize}
...
\noindent Tre livelli di \textbf{visibilità} degli attributi:
\begin{itemize}
    \item \textbf{+ pubblico}: accesso esteso a tutte le classi.
    \item \textbf{\# protetto}: accesso consentito solo alle classi che derivano dalla classe originale.
    \item \textbf{- privato}: solo la classe originale può accedere a tali attributi.
\end{itemize}
	
\noindent Il \textbf{nome} dell'attributo è l'\underline{unico parametro obbligatorio} ed il \textbf{tipo} può essere sia un tipo primitivo che una classe definita nello stesso diagramma.
	
\noindent La \textbf{molteplicità} indica il quantitativo degli attributi di un certo tipo: permette di indicare attributi come array o matrici, il valore default è 1. Il numero minimo e massimo possono essere racchiusi tra parentesi quadre quando il tipo è semplice. \\
...
\noindent \textbf{valoreDefault} è il valore assegnato all'attributo di default, se nessun valore è specificato durante la creazione. \\
\textbf{\{proprietà\}} indica caratteristiche aggiuntive dell'attributo (es: readOnly).
...
\noindent \textbf{Visibilità} e \textbf{nome} sono analoghi agli attributi, mentre, \textbf{tipoRestituito} è il tipo di valore di ritorno. \\
\noindent \textbf{listaParametri} contiene nome e tipo dei parametri della formula.

\begin{graybox}
\texttt{direzione nome parametro : tipo = valoreDefault}
\end{graybox}

\noindent \textbf{direzione}: input (in, rappresenta anche il valore default), output (out) o entrambi (inout). \\
\noindent \textbf{nome}, \textbf{tipo} e \textbf{valoreDefault} sono analoghi agli attributi.
...
\noindent Sono riportate come stringe di commento ( -- ) nel riquadro delle operazioni.
...
\begin{itemize}
    \item relazione di \textbf{associazione}.
    \item relazione di \textbf{generalizzazione-specializzazione}.
    \item relazione di \textbf{contenimento}.
\end{itemize}

\subsection{Associazioni}
L'associazione tra due classi esprime una relazione tra istanze delle classi, è caratterizzata da:
\begin{itemize}
    \item \textbf{nome}: esprime il legame semantico tra le classi associate.
    \item eventuale \textbf{ruolo} giocato da ciascuna delle parti associate.
...
\noindent Quando due classi sono coinvolte in associazioni differenti tra le stesse classi, per distinguerle è opportuno riportare un nome dell'associazione o il ruolo delle classi nelle diverse associazioni.
...
\noindent Se specificata con una \textbf{freccia} la direzionalità attribuisce alla classe origine del verso di percorrenza la responsabilità di tenere traccia dell'associazione.
...
\noindent Le associazioni, a differenza degli attributi, possono riportare anche le molteplicità di entrambe le classi ma sono meno compatte, il progettista deve scegliere le entità che hanno maggiore importanza e rappresentarle come classi per dar loro più enfasi. \\
\noindent Meglio non appesantire il diagramma con troppe associazioni 1 a 1, possibile usare \textbf{attributi} per concetti secondari e \textbf{associazioni} per classi significative, la scelta dipende da cosa si vuole enfatizzare nel programma.
...
\noindent La classe associativa connette due classi e definisce un insieme di caratteristiche proprie dell'associazione stessa, ogni istanza di classe associativa, rappresenta un'associazione tra due istanze delle classi e contiene specifici valori degli attributi. Dato che caratterizza un'associazione, prima di creare un'istanza della classe associativa, devono esistere le istanze delle classi collegate. \\
\noindent Ci può essere solo un'istanza di una specifica classe associativa tra ogni coppia di oggetti associati (istanze delle classi). \\
\noindent Per rendere reale una classe associativa sarò opportuno trasformarla in classe.

\subsection{Generalizzazione}
\subsubsection{Generalizzazione-Specializzazione (Gen-Spec)}
...
\noindent Tipo di associazione rappresentato da una freccia che va dalla classe specializzata alla classe generale (info comuni mantenute in un posto solo, facilitano le modifiche del progetto). \\
\noindent Le sottoclassi hanno tutti gli attributi e operazioni delle classi generali ed aggiungono operazioni e attributi più specifici. \\
\noindent A livello \textbf{concettuale} (analisi dei requisiti), gen-spec esprime una relazione is-a tra un concetto generale e le sue specializzazioni. \\
\noindent A livello di \textbf{dettaglio} (progetto e implementazione), gen-spec può essere interpretato:
\begin{itemize}
    \item come relazione di ereditarietà tra due classi concrete: le classi derivate ereditano attributi e metodi public e protected della classe base.
    \item come relazione di \textbf{realizzazione} tra una classe astratta e una classe concreta: la classe base ha soltanto prototipi di metodi, la classe specializzata implementa i metodi sfruttando l'overriding.
\end{itemize}

\noindent Quando Gen-Spec descrive una gerarchia di \textbf{ereditarietà}:
\begin{itemize}
    \item la superclasse raccoglie caratteristiche e comportamenti comuni agli elementi delle sottoclassi.
    \item la classe derivata (sottoclasse) eredita metodi e attributi della classe base (superclasse) e può:
    \begin{enumerate}
        \item aggiungere altri attributi e operazioni.
        \item ridefinire i metodi della classe base (overriding).
    \end{enumerate}
\end{itemize}
...
\begin{itemize}
    \item debole (lasco), detto anche \textbf{aggregazione}: la parte mantiene la sua identità quando entra a far parte del tutto, è modellata da un rombo vuoto.
    \item stretto, detto anche \textbf{composizione}: la parte perde la sua identità quando entra a far parte del tutto, è modellata da un rombo pieno.
\end{itemize}
...
\noindent Il ciclo di vita dell'oggetto contenuto è indipendente da quello dell'oggetto contenitore e può esistere indipendentemente dal contenitore. \\
\noindent L'oggetto contenitore non è necessariamente responsabile della costruzione e distribuzione dell'oggetto contenuto.
...
\noindent Le parti non esistono senza il tutto, se il composito viene distrutto anche le sue parti saranno distrutte (o responsabilità ceduta a un altro oggetto), il composito è responsabile della costruzione e distribuzione degli oggetti contenuti, ogni parte può appartenere ad un solo composito per volta.

\noindent \textbf{Principio di non condivisione} una classe può essere componente di più classi, un'istanza di classe componente può essere componente di un solo oggetto composito, anche se la classe componente può essere componente di molteplici classi composito diverse.

\noindent La molteplicità del lato del "tutto" (composito), quando sottintesa vale 1 nel caso in cui la classe componente ha solo un'altra classe composito.
